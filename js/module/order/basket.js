$(document).ready(function () {
  ...
  var fixedLayerPriceSet = function () {
    ...
    $('[id^="basket_chk_id_"]').each(function () {
      if ($(this).attr("checked") == true) {
        ...
        var iProductPrice =
          aBasketProductData[iCheckId].product_sum_price * iQuantity;
    ...
    if (iCheckPrdCnt > 0) {
      ...
      $("#checked_order_count")
        .html(
          "<strong>" +
            sprintf(__("%s"), iCheckPrdCnt) +
            "</strong>" +
            "개 상품선택"
        )
        .css("padding-bottom", "5px");
      $("#checked_order_price")
        .html("결제예정금액 <strong><em>" + sTotalPrice + "</em></strong>")
        .css("padding-bottom", "5px");

      var sPriceRef = SHOP_PRICE_FORMAT.shopPriceToSubPrice(iSumPrice);
      if (sPriceRef != "")
        $("#checked_order_price").find("strong").append(sPriceRef);
    } else {
      fixLayerPriceRest();
    }
  };
  ...
  var fixLayerPriceRest = function () {
    $("#checked_order_count, #checked_order_price")
      .html("")
